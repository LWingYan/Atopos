            ...
            <div class="comment-col" id="<?php $comments->theId(); ?>">
                <div class="comment-author" itemprop="creator" itemscope itemtype="http://schema.org/Person">
                    <div class="flex gap-2">
                        <!-- 评论者头像 -->
                        <span
                            itemprop="image" class="float-right rounded">
    ...
    <?php if ($this->allow('comment')): ?>
        <div id="<?php $this->respondId(); ?>" class="respond">
            ...
            <form method="post" action="<?php $this->commentUrl() ?>" id="comment-form" role="form">
                ...
                <textarea rows="8" cols="50" name="text" id="textarea" class=" textarea w-full text-slate-400 bg-sidebar rounded border box-border outline-0 p-2"
                ...
                <div class="flex gap-2 my-3">
                    <div class="w-full">
                        ...
                        <input type="text" name="author" id="author" class="w-full text text-slate-400 bg-sidebar rounded border box-border outline-0 p-2 "
                               value="<?php $this->remember('author'); ?>" required/>
                    </div>
                    <div class="w-full">
                        <p class="py-1.5 opacity-85"><label
                            for="mail" class="required text-slate-400 opacity-85 text-xs"><?php _e('Email'); ?></label></p>
                        <input type="email" name="mail" id="mail" class="w-full text text-slate-400 bg-sidebar rounded border box-border outline-0 p-2"
                               value="<?php $this->remember('mail'); ?>"<?php if ($this->options->commentsRequireMail): ?> required<?php endif; ?> />
                    </div>
                    <div class="w-full">
                        <p class="py-1.5 opacity-85"><label
                            for="url" class="required text-slate-400 opacity-85 text-xs"><?php _e('网站'); ?></label></p>
                        <input type="url" name="url" id="url" class="w-full text text-slate-400 bg-sidebar rounded border box-border outline-0 p-2" placeholder="<?php _e('http://'); ?>"
                               value="<?php $this->remember('url'); ?>"<?php if ($this->options->commentsRequireURL): ?> required<?php endif; ?> />
                    </div>
                </div>
                <?php endif; ?>
                <br>
                    <button type="submit" class="submit text-slate-400 bg-sidebar rounded border box-border outline-0 p-2"><?php _e('提交评论'); ?></button>
